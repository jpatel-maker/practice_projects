# Highly divisible triangular number problem
from math import floor, sqrt

# Generate a list of triangle numbers

#list of tiangle numbers

factors = 2
numbers = 1

# next triangle number starting from the first one
# for each natural number in a range , determine the triangle number to add to the the triangle numbers list.
# the nth triangle number is given by (n)(n+1)/2


def get_triangle_numbers (number):

    next_triangle_number = int((number)*(number+1)/2)
...
def get_factors (triangle_numbers):
...
# need to make more efficient
        ...
        for items in range(1, int(floor(sqrt(triangle_numbers)+1)) ):

            if triangle_numbers % items == 0:


                number_of_factors = number_of_factors +2


        return number_of_factors

print(get_factors(12375))
# next do this until we find a triangle number that has > 500 factors


while factors < 500:

    numbers = numbers + 1
    triangle_number = get_triangle_numbers(numbers)
...
# Project Euler problem 13: find the first ten digits of the sum of the following 100 50 digit numbers

# all 100 50 digit numbers

all_numbers = [3,7,1,0,7,2,8,7,5,3,3,9,0,2,1,0,2,7,9,8,7,9,7,9,9,8,2,2,0,8,3,7,5,9,0,2,4,6,5,1,0,1,3,5,7,4,0,2,5,0,
...
def get_sum (numbers):
    ...
    for index,digit in enumerate(numbers):
        ...
        if count_digits == 50:
            ...
            for values in current_number:

                number_value +=str (values)
